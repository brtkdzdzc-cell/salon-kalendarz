...
/**
 * GET /appointments?start=...&end=...&employeeId=...
 */
r.get("/", async (req, res) => {
...
/**
 * POST /appointments
 * UWAGA: brak blokad kolizji – pozwalamy na wiele wizyt w tym samym czasie
 */
...
r.patch("/:id", async (req, res) => {
  ...
  await db
    .prepare(
      `
      UPDATE appointments
      SET employee_id=?, client_id=?, start_at=?, end_at=?, service_name=?, notes=?, updated_at=NOW()
      WHERE id=?
    `
    )
    .run(employee_id, client_id, start_at, end_at, service_name, notes, id);
